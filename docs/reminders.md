...
При отправке напоминания в логах появляется:
```
LLM{user_id}REMINDER - {текст сообщения}
```
...
5. Отправляет напоминание
6. Обновляет `remind_of_yourself` на текущее время (09:05)
7. Следующее напоминание будет в 19:15
